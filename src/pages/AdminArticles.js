...
export default function AdminArticles() {
  ...
  const openEdit = (a) => {
    setForm({
      odooId: a.odooId, internalRef: a.internalRef, name: a.name,
      unit: a.unit, departments: [...(a.departments || ['all'])],
      category: a.category || '', active: a.active !== false,
    });
    setEditId(a.id); setFormError(''); setShowForm(true);
  };
    ...
    <div className="fade-in">
      ...
      <div className="articles-toolbar">
        <div className="toolbar-left">
          <input type="text" className="form-input" placeholder="Zoek naam, ref, categorie..."
          ...
          <table className="table">
            <thead>
              <tr>
                ...
                <th>Afdeling(en)</th><th>Odoo ID</th><th>Status</th><th></th>
              </tr>
            </thead>
            <tbody>
              {filtered.map(a => (
                <tr key={a.id} style={{ opacity: a.active ? 1 : 0.45 }}>
                  ...
                  <td>
                    <button className="status-toggle" onClick={() => toggleActive(a)}
      ...
      {showForm && (
        <Modal onClose={() => setShowForm(false)}
          title={editId ? 'Artikel bewerken' : 'Nieuw artikel'} maxWidth={540}>
          {formError && (
          ...
          <div className="article-form">
            ...
            <div className="form-field" style={{ gridColumn: '1 / -1' }}>
              ...
              <div className="dept-checkboxes">
                {DEPARTMENTS.map(d => (
                  <label key={d.value}
                    className={`dept-checkbox ${form.departments.includes(d.value) ? 'checked' : ''}`}>
                    <input type="checkbox" checked={form.departments.includes(d.value)}
