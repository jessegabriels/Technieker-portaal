...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (!username || !password) return cors({ error: 'Gebruikersnaam en wachtwoord zijn verplicht.' }, 400);

    const user = await findByUsername(username.toLowerCase().trim());
    if (!user || !user.active) return cors({ error: 'Ongeldige inloggegevens.' }, 401);

    if (hashPassword(password) !== user.passwordHash) return cors({ error: 'Ongeldige inloggegevens.' }, 401);
